...
| 2026-08-04 | **Sealed notes merged (b5d9b4e)** — `lock`/`basslock` on the page, "prl" seal combos, refusals in the footer's own voice, gilt rings + ticks in both views; the primitive under the revised 3.2 board. Priced **medium = both banked smalls** (judged honestly: new schema field + enforcement at every write door + two renderers). Free alongside: moveNote seal-bypass seam (warranty, b62d729), seals-strip repair (warranty, 80b399b), the revised board itself (curriculum). | 1 credit |
| 2026-08-04 | **□ favourite on the pad (43570bb)** — □ keeps a quest to hand while the log is up, resting ✧ mark on every row so the button can be found. Judged honestly at request time: one binding plus one glyph, **half-small = the 1 banked credit**. Bass re-voicing to a small spectrum (06cb71f): warranty. | **0 — tapped out** |
